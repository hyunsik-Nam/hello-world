...
<input type="hidden" name="btype" value="${param.btype}">
	...
	<a href = "list.do?btype=${param.btype}" style="text-decoration:none">목록</a>
...
<c:forEach var="r" items="${rlist1}">
   ...
   <c:if test="${sessionScope.nick == r.nick}">
   	 ...
   	 <td width="7%">
   			<font size="1">
    			 <a href="javascript:$('#updateform${r.no}').toggle()" style="text-align:rignt; text-decoration:none;">수정</a>
   		    </font>
     </td>
...
<table colspan="2" style="width:60%; margin: auto;">
	...
	<td colspan="0.5" width="20%"><a href="javascript:document.d.submit()">댓글수정</a></td></tr>
...
<table colspan="2" style="width:60%; margin: auto;">
	...
	<c:if test="${empty sessionScope.login}">
	<td colspan="0.5" width="20%"><a href = "info.do?num=${b.num}&btype=${b.btype}" onclick="check()">댓글등록</a></td></tr>
	</c:if>
	<c:if test="${!empty sessionScope.login}">
