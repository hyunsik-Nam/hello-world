...
public class MemberDao {
	...
	public int insert(Member mem) {
		...
		// 문장을 준비하는 객체
		String sql = "insert into member" + " (id,pass,name,nick,email,picture)" + " value (?,?,?,?,?,?)";
		try {
	...
	public String idSearch(String email, String name) {
		...
		try {
			return session.getMapper(cls).idSearch(email,name);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			MyBatisConnection.close(session);
	...
	public String pwSearch(String id, String email, String name) {
		SqlSession session = MyBatisConnection.getConnection();
		String sql = "select id from member where id =? and email = ? and name = ?";
		try {
			
			return session.getMapper(cls).passSearch(id,email,name);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			MyBatisConnection.close(session);
